  ...
  return (
    <div className="min-h-screen bg-[#0d0f14]">
      <div className="fixed inset-0 opacity-5 pointer-events-none"
        style={{ backgroundImage: "linear-gradient(#2a2d35 1px, transparent 1px), linear-gradient(90deg, #2a2d35 1px, transparent 1px)", backgroundSize: "32px 32px" }} />

      <div className="relative max-w-3xl mx-auto px-4 py-12">
        {/* Nav */}
        <div className="flex items-center gap-4 mb-8">
          <Link href="/dashboard" className="text-[8px] text-gray-500 hover:text-amber-400 transition-colors"
            style={{ fontFamily: "'Press Start 2P', monospace" }}>
            ← DASHBOARD
          </Link>
        </div>

        <h1 className="text-sm text-white mb-2" style={{ fontFamily: "'Press Start 2P', monospace" }}>
          <span className="text-amber-400">▶</span> API KEYS
        </h1>
        <p className="text-xs text-gray-400 mb-8">Use these keys to query hermitcrab from your agent or app.</p>
        ...
        {/* Existing keys */}
        <div className="bg-[#161920] border-2 border-[#2a2d35] p-6"
          style={{ boxShadow: "2px 2px 0px #000" }}>
          <h2 className="text-[9px] text-amber-400 mb-4" style={{ fontFamily: "'Press Start 2P', monospace" }}>
            ACTIVE KEYS
          </h2>
          ...
          {loading ? (
            <p className="text-[9px] text-gray-500 animate-pulse" style={{ fontFamily: "'Press Start 2P', monospace" }}>LOADING...</p>
          ) : keys.length === 0 ? (
            <p className="text-xs text-gray-500">No keys yet. Generate one above.</p>
          ) : (
            <div className="space-y-3">
              {keys.map((key) => (
                <div key={key.key_prefix}
                  className="flex items-center justify-between gap-4 bg-[#0d0f14] border border-[#2a2d35] p-4">
                  <div>
                    <p className="text-[9px] text-white" style={{ fontFamily: "'Press Start 2P', monospace" }}>{key.name}</p>
                    <p className="text-[8px] text-gray-500 mt-1 font-mono">{key.key_prefix}...</p>
                    <p className="text-[7px] text-gray-600 mt-1">Created {new Date(key.created_at).toLocaleDateString()}</p>
                  </div>
                  <button
        ...
        {/* Usage example */}
        <div className="mt-6 bg-[#161920] border-2 border-[#2a2d35] p-6"
          style={{ boxShadow: "2px 2px 0px #000" }}>
          <h2 className="text-[9px] text-amber-400 mb-4" style={{ fontFamily: "'Press Start 2P', monospace" }}>
            USAGE
          </h2>
